...
UPDATE 2026-07-10: the breadcrumbs answered — the (b) gate has confirmed false positives on quiet trailing speech. A dozen `segments_filtered` drops since 2026-07-09 are clearly real dictation ("Like, what would be the best?", "Check out Nice Schedule.", "And we emailed that out."), all no_speech 0.6–0.75 / logprob −0.45–−0.74: a soft-spoken final sentence looks like silence to the encoder and decodes just under the logprob bar. This presented to the user as "the end of what I said got cut off" (compounding the capture-side truncation fixed the same day: stop grace tail + asymmetric trim pad + Whisper tail pad). Fix, three layers: (a) `_auto_gain` AGC-lite normalizes quiet takes toward −24 dBFS speech level (95th-pct windowed RMS, +21.6 dB cap, clip headroom) before any threshold sees the audio; (b) energy rescue — `_filter_result_segments` now measures the actual audio in a gate-tripping segment's time range (`_segment_sounds_voiced`) and keeps it when it carries real energy (real quiet speech ≠ confabulation over near-zero signal), breadcrumbed as `segment_kept_voiced`; (c) new `mic_sensitivity` setting (high/normal/low, default normal) in both settings UIs — "high" loosens the trim/segment gates for quiet talkers, "low" tightens them for noisy rooms. Watch for the inverse regression: a hallucination over a loud noise bed getting rescued by (b) — the noise-floor multiplier in `_segment_sounds_voiced` is the guard.
